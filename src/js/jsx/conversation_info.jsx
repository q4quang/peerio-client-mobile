    ...
    componentWillMount: function(){
      Peerio.Files.getAllFiles()
          .then(function (files) {
            ...
            this.findAllConversationFiles(files);
          }.bind(this));
      this.setState({conversation: Peerio.Messages.cache[this.props.params.id], files:[]});
    },
    findAllConversationFiles: function(allUserFiles){
      var files = [];
      ...
      return (
          <div className="content-padded">
            ...
            <h2 className="subhead">
              ...
              <span className="icon-with-label"><i className="fa fa-comment-o"/>&nbsp;{info.messageCount}</span>
              <span className="icon-with-label"><i className="fa fa-file-o"/>&nbsp;{info.fileCount}</span>
              <span className="icon-with-label"><i className="fa fa-users"/>&nbsp;{info.participantCount}</span>
            </h2>
        ...
        <Peerio.UI.Tappable onTap={this.openContactView.bind(this, this.state.contact.username)}>
          <div className="contact list-item">
            ...
            <div className="list-item-content">
              <span className={removed ? 'text-crossout':''}>{this.state.contact.fullName} ({this.state.contact.username})</span>
              {eventInfo}
            </div>
          </div>
        </Peerio.UI.Tappable>
