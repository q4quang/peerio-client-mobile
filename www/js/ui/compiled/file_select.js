...
(function () {
  ...
  Peerio.UI.FileSelect = React.createClass({displayName: "FileSelect",
    ...
    render: function () {
      ...
      if (Peerio.user.filesLoaded) {
        _.forOwn(Peerio.user.files, function (f) {
          ...
          files.push(
            React.createElement("li", {className: "contact", "data-fileid": f.id, key: f.localName}, 
            checkMark, 
              React.createElement("span", {className: "username"}, f.name)
            )
          );
        }.bind(this));
      } else {
        files = React.createElement("li", null, "Loading files, please wait...");
      }

      return (
        React.createElement("ul", {className: "contact-select file-select", onTouchStart: this.registerTouchStart, onTouchEnd: this.registerTouchEnd}, 
          files
        )
      );
    }
